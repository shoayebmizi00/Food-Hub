...
export default function AdminCategories() {
  ...
  const load = async () => { setCategories(await api.entities.FoodCategory.filter({ restaurant_id: undefined }, 'sort_order', 50).catch(() => api.entities.FoodCategory.list('sort_order', 50))); };
  useEffect(() => { load(); }, []);

  const handleSave = async () => {
    if (!form.name) return;
    if (editing) { await api.entities.FoodCategory.update(editing.id, form); } else { await api.entities.FoodCategory.create({ ...form, is_active: true }); }
    setShowForm(false); setEditing(null); setForm({ name: '', icon: '', sort_order: 0 });
    toast({ title: editing ? 'Category updated' : 'Category created' }); load();
  };

  const handleDelete = async (id) => { await api.entities.FoodCategory.delete(id); toast({ title: 'Deleted' }); load(); };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Categories</h1>
        <Button onClick={() => { setEditing(null); setForm({ name: '', icon: '', sort_order: 0 }); setShowForm(true); }} className="rounded-xl gap-2"><Plus className="w-4 h-4" /> Add</Button>
      </div>
          ...
          <div key={c.id} className="bg-card rounded-2xl border border-border p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Order: {c.sort_order || 0}</p>
              </div>
            </div>
            <div className="flex gap-1">
              ...
              <button onClick={() => handleDelete(c.id)} className="p-2 rounded-lg hover:bg-muted text-destructive"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
